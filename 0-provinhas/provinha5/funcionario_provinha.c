...
void altera_nome(FILE *in, int i, char* novo_nome) {
	...
	//pula primeiros (i - 1) registros para posicionar no início i-ésimo resgistro
	fseek(in, tamanho() * (i-1), SEEK_SET);		// SEEK_SET => deslocamento a partindo inicio
...
int main(int argc, char** argv) {
	...
	if ((out = fopen("funcionario.dat", "w+b")) == NULL) {
		...
		altera_ultimo_nome(out, "Taiane ultimo");
		le_funcionarios(out);
		//fecha arquivo
		fclose(out);
	}
